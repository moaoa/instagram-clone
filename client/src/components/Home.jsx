...
export default function Home() {
  ...
  const sendComment = (postId, text) => {
    ...
    // fakeApi.put("/comment", { postId, text }).then((res) => {
    //   console.log(res);
    //   if (res.status === 200) return addComment(res.data.post);
    // });
  };
        ...
        <div className="card" key={i}>
          ...
          <div className="card-content">
            <div className="icons-box">
              <span className="likes-counter">{post?.likes.length} likes</span>
            ...
            {post.comments.map((comment) => (
              <div className="comment" key={post._id}>
                {comment.text} <strong>by {comment.createdBy.name}</strong>
              </div>
            ))}
            <form
              onSubmit={(e) => {
